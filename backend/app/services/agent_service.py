...
class AgentService:
    ...
    async def stream_chat(self, request: ChatRequest) -> AsyncGenerator[dict[str, Any], None]:
        ...
        history = [
            {"role": msg.role.value, "content": msg.content} for msg in request.messages[:-1]
        ]

        current_message_text = request.messages[-1].content

        if request.image_base64:
            parsed_bill = await self._agent_tools.tool_parse_bill(
                {"image_base64": request.image_base64}
            )
            yield {
                "type": "tool_call",
                "data": {"tool": "parse_utility_bill", "result": parsed_bill},
            }
        ...
        messages: list[Any] = [
            ...
            *history,
            {"role": "user", "content": context_prefix + current_message},
        ]
        ...
        try:
            ...
            if tool_calls:
                ...
                for tc in tool_calls.values():
                    ...
                    tool_result = await self._agent_tools.dispatch_tool(
                        fn_name,
                        fn_args,
                        request,
                        constraints,
    ...
    async def parse_simulate_query(self, query: str) -> list[dict[str, Any]]:
                ...
                "scenarios": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "new_heating_type": {
                                "type": "string",
                                "enum": ["gas", "oil", "electric", "heat_pump", "biomass"],
                            },
                            ...
                            "new_device_frequency": {
                                "type": "string",
                                "enum": ["frequent", "average", "rare"],
                            },
                            "new_streaming_usage": {
                                "type": "string",
        ...
        safe_query = (
            query.replace('"""', "'").replace("<system>", "").replace("</system>", "").strip()
        )
