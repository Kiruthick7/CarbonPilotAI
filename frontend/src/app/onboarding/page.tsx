...
export default function OnboardingPage() {
  ...
  const handleManualSubmit = async (
    ...
    setError(null);
    try {
      const parsedValue = parseFloat(valueToUse);
      if (isNaN(parsedValue)) {
        throw new Error("Invalid number provided for kWh.");
      }
      const data = await carbonApi.submitManualEntry(parsedValue);
      setTempOcrData(data);
      setStep("digital");
    } catch (err: unknown) {
      setError(getErrorMessage(err) || "An unexpected error occurred.");
  ...
  const handleLoadDemoProfile = () => {
    setTempOcrData(DEMO_CARBON_PROFILE_DATA);
    setStep("digital");
  };
  ...
  const handleDigitalSubmit = async () => {
    ...
    try {
      ...
      const data = await carbonApi.calculateDigitalFootprint(
        updatedProfile as CarbonProfile,
      );
      setOcrData({
        ...tempOcrData,
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-200/50 p-10">
        ...
        {step === "digital" ? (
          ...
          <>
            <StepNavigation entryMode={entryMode} setEntryMode={setEntryMode} />
            ...
            {entryMode === "manual" ? (
              ...
              <>
                ...
                <div className="mt-4 mb-6 text-xs text-slate-500 flex items-center justify-center gap-2">
                  ...
                  <p>
                    <strong>Privacy Guarantee:</strong> Processed securely via
                    in-memory server instances. All files are instantly deleted
                    after extraction and never stored.
                  </p>
                </div>
                ...
                <div className="mt-6 mb-4 flex items-start gap-3">
                  ...
                  >
                    I consent to processing my uploaded files using OCR to
                    extract energy consumption data. Files are processed
                    in-memory and are <strong>never stored</strong> in any
                    database.
                  </label>
                </div>
