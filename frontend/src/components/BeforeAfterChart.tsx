...
export function BeforeAfterChart({ originalBreakdowns, newBreakdowns }: BeforeAfterChartProps) {
  // Find max value to scale the bars relative to the largest category
    ...
    <div className="w-full text-left mt-8 bg-black/20 p-6 rounded-2xl border border-white/5 backdrop-blur-md animate-in fade-in duration-500 slide-in-from-bottom-4">
      ...
      <div className="space-y-5">
        {originalBreakdowns.map((orig, idx) => {
          const newBreakdown = newBreakdowns.find((b) => b.category === orig.category);
          const newKg = newBreakdown ? newBreakdown.total_kgco2e : orig.total_kgco2e;

          // Don't show categories with 0 baseline to keep it clean
          ...
          const reductionPercent = ((orig.total_kgco2e - newKg) / orig.total_kgco2e) * 100;

          return (
            <div key={idx} className="w-full group">
              <div className="flex justify-between items-end mb-1.5">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-slate-300 capitalize">{orig.category}</span>
                  {reduced && (
                ...
                <div className="text-xs font-mono text-right flex gap-2">
                  <span className={reduced ? "text-slate-500 line-through" : "text-slate-400"}>
